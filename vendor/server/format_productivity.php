<?php
    ...
    function productivity_report_format($conn)
    {
        ...
        for ($row = 2; $row <= $product_highestRow; $row++) {
            ...
            $cutoff_type = gettype($cutoff_date);
            if ($cutoff_type==='string') {
                $cutoff_date = date_create_from_format('m/d/Y', $cutoff_date);
                $cutoff_date = $cutoff_date->format('d/m/Y');
            } else {
                $cutoff_date = date('d/m/Y', PHPExcel_Shared_Date::ExcelToPHP($cutoff_date));
            }
            $cutoff_date = date_create_from_format('d/m/Y', $cutoff_date);
            $current_date = date('d/m/Y');
            $current_date = date_create_from_format('d/m/Y', $current_date);
            $dayer = date_diff($cutoff_date, $current_date)->format("%a");
            $cutoff_date = $cutoff_date->format('d/m/Y');
            $pentadas = round($dayer / 5);
